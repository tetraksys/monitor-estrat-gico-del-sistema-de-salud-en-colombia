...
import { PAIS_OPTIONS } from '../constants';


const Dashboard: React.FC = () => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [reportData, setReportData] = useState<ReportData | null>(null); const [groundingSources, setGroundingSources]=useState<GroundingSource[]>([]); const [error, setError]=useState<string | null>(null); const [selectedPais, setSelectedPais]=useState<string>(PAIS_OPTIONS[0]); const handleGenerateReport=useCallback(async ()=> {
    setIsLoading(true);
    setError(null);
    setReportData(null);
    setGroundingSources([]);
    ...
    try {
      const result = await generateReport(selectedPais);
  ...
  return (
    <div className="container mx-auto">
			<div className="bg-white p-6 rounded-lg shadow-lg border border-slate-200">
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
					...
					<div className="flex items-center space-x-4">
						<div>
							<label htmlFor="pais-select" className="sr-only">País</label>
							<select id="pais-select" value={selectedPais} onChange={(e)=> setSelectedPais(e.target.value)}
                className="bg-white border border-slate-300 rounded-md py-2 px-3 text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
            ...
            <button onClick={handleGenerateReport} disabled={isLoading} className="flex items-center justify-center bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white font-bold py-2 px-4 rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
              {isLoading ? (
                <>
								<Spinner/>
                  Generando...
  ...
  Policy insight developed by{' '}
  <a href="https://www.linkedin.com/in/juliopuentes/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline">
    Julio César Puentes
  </a>
				<br/>
  Engineering by{' '}
  <a href="https://www.linkedin.com/in/javiersamirreyr/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline">
    Javier Rey
  </a>
				<br/>
